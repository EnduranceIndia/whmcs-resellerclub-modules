...
/**
 * WHMCS Module for Resellerclub's Reseller Hosting - India ( Linux / Windows )
 */
// AddOn Module Name
 ...
 */
global $orderbox;
$orderbox = new orderboxapi($resellerclub_config['resellerid'], $resellerclub_config['apikey'], $resellerclub_config['enable_log'], 'rc-whmcs-resellerclubresellerhosting-in');

/**
 * WHMCS provisional module core functions
 */
function resellerclubresellerhostingin_ConfigOptions() {

    global $orderbox;
    $hosting_plan_names = array();
    $active_products = $orderbox->api('GET', '/products/category-keys-mapping.json', array(), $response, 'resellerclubresellerhostingin', 'configoptions');

    if (in_array('resellerhostinglinuxin', $active_products['hosting']) || in_array('resellerwindowshostingin', $active_products['hosting'])) {
        $plans = $orderbox->api('GET', '/products/plan-details.json', array(), $response, 'resellerclubresellerhostingin', 'configoptions');

        if (array_key_exists('resellerhostinglinuxin', $plans) && in_array('resellerhostinglinuxin', $active_products['hosting'])) {
            $hosting_plans = $plans['resellerhostinglinuxin'];
            foreach ($hosting_plans as $plan_id => $plan) {
                $hosting_plan_names[] = 'Linux ' . ' - ' . $plan['plan_name'] . ' - ' . $plan_id;
            }
        }
        if (array_key_exists('resellerwindowshostingin', $plans) && in_array('resellerwindowshostingin', $active_products['hosting'])) {
            $hosting_plans = $plans['resellerwindowshostingin'];
            foreach ($hosting_plans as $plan_id => $plan) {
...
function resellerclubresellerhostingin_CreateAccount($params) {

    global $orderbox;

    try {
        $client_details = $orderbox->api('GET', '/customers/details.json', array('username' => $params['clientsdetails']['email']), $response, 'resellerclubresellerhostingin', 'create');
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_order_add = '/resellerhosting/windows/in/add.json';
        } else {
            $api_path_order_add = '/resellerhosting/linux/in/add.json';
        }

        $order_api_result = $orderbox->api('POST', $api_path_order_add, $order_details, $response, 'resellerclubresellerhostingin', 'create');
        ...
        logModuleCall('resellerclubresellerhostingin', 'create', 'unknown', 'Exception : ' . $e->getMessage());
        return "Customer sign up error - " . $e->getMessage();
    }
}

function resellerclubresellerhostingin_SuspendAccount($params) {
    ...
    try {
        $plan_pieces = _get_plan_details($params['configoption1']);
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/in/orderid.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/in/orderid.json';
        }

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostingin', 'suspend');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_suspend_result = $orderbox->api('POST', '/orders/suspend.json', array('order-id' => $order_id, 'reason' => $params['suspendreason']), $response, 'resellerclubresellerhostingin', 'suspend');
        ...
        logModuleCall('resellerclubresellerhostingin', 'suspend', 'unknown', 'Exception : ' . $e->getMessage());
        return "Order suspend error - " . $e->getMessage();
    }
}

function resellerclubresellerhostingin_UnsuspendAccount($params) {
    ...
    try {
        ...
        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostingin', 'unsuspend');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_unsuspend_result = $orderbox->api('POST', '/orders/unsuspend.json', array('order-id' => $order_id), $response, 'resellerclubresellerhostingin', 'unsuspend');
        ...
        logModuleCall('resellerclubresellerhostingin', 'unsuspend', 'unknown', 'Exception : ' . $e->getMessage());
        return "Order unsuspend error - " . $e->getMessage();
    }
}

function resellerclubresellerhostingin_TerminateAccount($params) {
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/in/orderid.json';
            $api_path_order_delete = '/resellerhosting/windows/in/delete.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/in/orderid.json';
            $api_path_order_delete = '/resellerhosting/linux/in/delete.json';
        }


        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostingin', 'terminate');

        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            return $order_id_result['message'];
        } else {
            $order_id = $order_id_result;
            $order_delete_result = $orderbox->api('POST', $api_path_order_delete, array('order-id' => $order_id), $response, 'resellerclubresellerhostingin', 'terminate');
...
function resellerclubresellerhostingin_Renew($params) {
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/in/orderid.json';
            $api_path_order_renew = '/resellerhosting/windows/in/renew.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/in/orderid.json';
            $api_path_order_renew = '/resellerhosting/linux/in/renew.json';
        }

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostingin', 'renew');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_api_result = $orderbox->api('POST', $api_path_order_renew, $order_details, $response, 'resellerclubresellerhostingin', 'renew');
...
function resellerclubresellerhostingin_manualrenew($params) {

    //Renew the order
    $renew_result = resellerclubresellerhostingin_Renew($params);
...
function resellerclubresellerhostingin_ChangePackage($params) {
    global $orderbox;
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/in/orderid.json';
            $api_path_order_modify = '/resellerhosting/windows/in/modify.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/in/orderid.json';
            $api_path_order_modify = '/resellerhosting/linux/in/modify.json';
        }
        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostingin', 'changepackage');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_api_result = $orderbox->api('POST', $api_path_order_modify, $order_details, $response, 'resellerclubresellerhostingin', 'changepackage');
...
function resellerclubresellerhostingin_ClientAreaCustomButtonArray() {
    
}

function resellerclubresellerhostingin_AdminCustomButtonArray() {
    return array("Execute Manual Renew" => "manualrenew");
}

function resellerclubresellerhostingin_ClientArea($params) {
    ...
    if (isset($_POST['cplogin']) && strlen(trim($_POST['cplogin'])) > 0) {
        ...
        switch ($cplogin_action) {
            ...
            case 'whm': _redirect_to_whm_control_panel($params);
                break;
            case 'whmcs_license': resellerclubresellerhostingin_GenerateLicense($params);
                break;
        }
    }
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/in/orderid.json';
            $api_path_order_details = '/resellerhosting/windows/in/details.json';
            $api_path_dns_details = '/resellerhosting/windows/in/dns-record.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/in/orderid.json';
            $api_path_order_details = '/resellerhosting/linux/in/details.json';
            $api_path_dns_details = '/resellerhosting/linux/in/dns-record.json';
        }

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostingin', 'clientarea');

        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            $is_processing = true;
        } else {
            $order_id = $order_id_result;
            $order_details = $orderbox->api('GET', $api_path_order_details, array('order-id' => $order_id), $response, 'resellerclubresellerhostingin', 'clientarea');
            $order_dns_details = $orderbox->api('GET', $api_path_dns_details, array('order-id' => $order_id), $response, 'resellerclubresellerhostingin', 'clientarea');
        }
        ...
        if ($is_processing) {
            ...
            if ($plans['resellerhostinglinuxin'][$plan_pieces['id']]['includes_whmcs'] == true) {
...
function resellerclubresellerhostingin_LoginLink($params) {
    echo "<strong>Do Not Modify</strong>" . _display_control_panel_link($params);
}

function resellerclubresellerhostingin_GenerateLicense($params) {
    ...
    try {
        $plan_pieces = _get_plan_details($params['configoption1']);
        if ('windows' == $plan_pieces['type']) {
            $api_path = '/resellerhosting/windows/in/orderid.json';
            $api_path_generate_license_key = '/resellerhosting/windows/in/generate-license-key.json';
        } else {
            $api_path = '/resellerhosting/linux/in/orderid.json';
            $api_path_generate_license_key = '/resellerhosting/linux/in/generate-license-key.json';
        }
        $order_id_result = $orderbox->api('GET', $api_path, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostingin', 'generateLicense');

        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            return $order_id_result['message'];
        } else {
            $order_id = $order_id_result;
            $license_details = $orderbox->api('POST', $api_path_generate_license_key, array('order-id' => $order_id), $response, 'resellerclubresellerhostingin', 'generateLicense');
...
if (!function_exists('_createCustomer')) {
    ...
    function _createCustomer($params) {
        ...
        $create_customer_result = $orderbox->api('POST', '/customers/signup.json', $customer_details, $response_headers, 'resellerclubresellerhostingin', 'create');
...
if (!function_exists('_get_control_panel_link')) {
    ...
    function _get_control_panel_link($params) {
        ...
        $client_details = $orderbox->api('GET', '/customers/details.json', array('username' => $params['clientsdetails']['email']), $response, 'resellerclubresellerhostingin', 'clientarea');
        if (is_array($client_details) && strtolower($client_details['status']) == 'error') {
            $error = "Customer ({$params['clientsdetails']['email']}) not found at Resellerclub";
        } else {
            $customer_id = $client_details['customerid'];
            $customer_temp_password = $orderbox->api('GET', '/customers/temp-password.json', array('customer-id' => $customer_id), $response, 'resellerclubresellerhostingin', 'clientarea');
        ...
        if ($error == '') {
            ...
            $reseller_details = $orderbox->api('GET', '/resellers/details.json', array(), $response, 'resellerclubresellerhostingin', 'clientarea');
        ...
        if ($error == '') {
            ...
            $authentication_token_result = $orderbox->api('GET', '/customers/generate-token.json', array('username' => $params['clientsdetails']['email'], 'passwd' => $resellerclub_customer_password, 'ip' => $ip), $response, 'resellerclubresellerhostingin', 'clientarea');
