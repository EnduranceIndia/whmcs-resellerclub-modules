...
/**
 * WHMCS Module for Resellerclub's webservices email, dyi
 */
// AddOn Module Name
 ...
 */
global $orderbox;
$orderbox = new orderboxapi($resellerclub_config['resellerid'], $resellerclub_config['apikey'], $resellerclub_config['enable_log'], 'rc-whmcs-resellerclubwebservices');

/**
 * WHMCS provisional module core functions
 */
function resellerclubwebservices_ConfigOptions() {

    global $orderbox;
    $hosting_plan_names = array();

    $plans = $orderbox->api('GET', '/products/plan-details.json', array(), $response, 'resellerclubwebservices', 'configoptions');
    $hosting_plans = $plans['hosting'];

    foreach ($hosting_plans as $plan_id => $plan) {
        if ('Active' == $plan['status']) {
            switch ($plan['category']) {
                case 'websitebuilder_plan':
                    $hosting_plan_names[] = 'Website builder ' . ' - ' . $plan['plan_name'] . ' - ' . $plan_id;
                    break;
                case 'email_plan':
                    $hosting_plan_names[] = 'Email ' . ' - ' . $plan['plan_name'] . ' - ' . $plan_id;
                    break;
            }
        }
    }
    if (empty($hosting_plan_names)) {
        $configarray = array(
            "Web Services Plan" => array("Description" => "No plans active for selected module")
        );
    } else {
        $configarray = array(
            "Web Services Plan" => array("Type" => "dropdown", "Options" => implode(',', $hosting_plan_names), "Description" => "Select a web service plan to associate with this product")
        );
    }
...
function resellerclubwebservices_CreateAccount($params) {

    global $orderbox;

    try {
        $client_details = $orderbox->api('GET', '/customers/details.json', array('username' => $params['clientsdetails']['email']), $response, 'resellerclubwebservices', 'create');
        ...
        $api_path_order_add = '/webservices/add.json';


        $order_api_result = $orderbox->api('POST', $api_path_order_add, $order_details, $response, 'resellerclubwebservices', 'create');
        ...
        logModuleCall('resellerclubwebservices', 'create', 'unknown', 'Exception : ' . $e->getMessage());
        return "Customer sign up error - " . $e->getMessage();
    }
}

function resellerclubwebservices_SuspendAccount($params) {
    ...
    try {
        ...
        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubwebservices', 'suspend');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_suspend_result = $orderbox->api('POST', '/orders/suspend.json', array('order-id' => $order_id, 'reason' => $params['suspendreason']), $response, 'resellerclubwebservices', 'suspend');
        ...
        logModuleCall('resellerclubwebservices', 'suspend', 'unknown', 'Exception : ' . $e->getMessage());
        return "Order suspend error - " . $e->getMessage();
    }
}

function resellerclubwebservices_UnsuspendAccount($params) {
    ...
    try {
        ...
        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubwebservices', 'unsuspend');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_unsuspend_result = $orderbox->api('POST', '/orders/unsuspend.json', array('order-id' => $order_id), $response, 'resellerclubwebservices', 'unsuspend');
        ...
        logModuleCall('resellerclubwebservices', 'unsuspend', 'unknown', 'Exception : ' . $e->getMessage());
        return "Order unsuspend error - " . $e->getMessage();
    }
}

function resellerclubwebservices_TerminateAccount($params) {
    ...
    try {
        ...
        $api_path_orderid_from_domain = '/webservices/orderid.json';
        $api_path_order_delete = '/webservices/delete.json';

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubwebservices', 'terminate');

        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            return $order_id_result['message'];
        } else {
            $order_id = $order_id_result;
            $order_delete_result = $orderbox->api('POST', $api_path_order_delete, array('order-id' => $order_id), $response, 'resellerclubwebservices', 'terminate');
...
function resellerclubwebservices_Renew($params) {
    ...
    try {
        ...
        $api_path_orderid_from_domain = '/webservices/orderid.json';
        $api_path_order_renew = '/webservices/renew.json';

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubwebservices', 'renew');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_api_result = $orderbox->api('POST', $api_path_order_renew, $order_details, $response, 'resellerclubwebservices', 'renew');
...
function resellerclubwebservices_manualrenew($params) {

    //Renew the order
    $renew_result = resellerclubwebservices_Renew($params);
...
function resellerclubwebservices_ChangePackage($params) {

    global $orderbox;

    try {
        $api_path_orderid_from_domain = '/webservices/orderid.json';
        $api_path_order_modify = '/webservices/modify.json';

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubwebservices', 'changepackage');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_api_result = $orderbox->api('POST', $api_path_order_modify, $order_details, $response, 'resellerclubwebservices', 'changepackage');
...
function resellerclubwebservices_ClientAreaCustomButtonArray() {
    
}

function resellerclubwebservices_AdminCustomButtonArray() {
    ...
    if (isset($_POST['cplogin']) && strlen(trim($_POST['cplogin'])) > 0) {
        ...
        switch ($cplogin_action) {
            ...
            case 'webmail': _redirect_to_webmail_control_panel($params);
                break;
            case 'websitebuilder': _redirect_to_websitebuilder_control_panel($params);
                break;
        }
    }
...
function resellerclubwebservices_LoginLink($params) {
...
if (!function_exists('_createCustomer')) {
    ...
    function _createCustomer($params) {
        ...
        $create_customer_result = $orderbox->api('POST', '/customers/signup.json', $customer_details, $response_headers, 'resellerclubwebservices', 'create');
...
if (!function_exists('_get_control_panel_link')) {
    ...
    function _get_control_panel_link($params) {
        ...
        $client_details = $orderbox->api('GET', '/customers/details.json', array('username' => $params['clientsdetails']['email']), $response, 'resellerclubwebservices', 'clientarea');
        if (is_array($client_details) && strtolower($client_details['status']) == 'error') {
            $error = "Customer ({$params['clientsdetails']['email']}) not found at Resellerclub";
        } else {
            $customer_id = $client_details['customerid'];
            $customer_temp_password = $orderbox->api('GET', '/customers/temp-password.json', array('customer-id' => $customer_id), $response, 'resellerclubwebservices', 'clientarea');
        ...
        if ($error == '') {
            ...
            $reseller_details = $orderbox->api('GET', '/resellers/details.json', array(), $response, 'resellerclubwebservices', 'clientarea');
        ...
        if ($error == '') {
            ...
            $authentication_token_result = $orderbox->api('GET', '/customers/generate-token.json', array('username' => $params['clientsdetails']['email'], 'passwd' => $resellerclub_customer_password, 'ip' => $ip), $response, 'resellerclubwebservices', 'clientarea');
...
if (!function_exists('_redirect_to_websitebuilder_control_panel')) {

    function _redirect_to_websitebuilder_control_panel($params) {
        $control_panel_url = _get_control_panel_link($params) . '&service-name=websitebuilder';
        header("location: " . $control_panel_url);
        exit;
    }
...
if (!function_exists('_display_websitebuilder_panel_form')) {

    function _display_websitebuilder_panel_form() {
        $form_action_url = $_SERVER['REQUEST_URI'];
        $id = isset($_GET['id']) ? $_GET['id'] : ( isset($_POST['id']) ? $_POST['id'] : '' );
        $cp_form = "<form method=\"post\" action=\"{$form_action_url}\" target=\"_blank\">";
        $cp_form .= "<input type=\"hidden\" name=\"id\" value=\"" . $id . "\">";
        $cp_form .= "<input type=\"hidden\" name=\"cplogin\" value=\"websitebuilder\">";
        $cp_form .= "<input type=\"submit\" name=\"btn_cplogin\" value=\"Web Site Builder\">";
        $cp_form .= "</form>";
        return $cp_form;
    }
