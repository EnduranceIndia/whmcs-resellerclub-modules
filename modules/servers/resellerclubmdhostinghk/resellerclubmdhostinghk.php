...
/**
 * WHMCS Module for Resellerclub's Multi Domain Hosting - Hong Kong ( Linux / Windows )
 */
...
global $orderbox;
$orderbox =  new orderboxapi( $resellerclub_config['resellerid'] , $resellerclub_config['apikey'],$resellerclub_config['enable_log'], 'rc-whmcs-resellerclubmdhosting-hk' );

/**
 * WHMCS provisional module core functions
 */

function resellerclubmdhostinghk_ConfigOptions() {

    global $orderbox;
    $hosting_plan_names = array();
    
    $active_products = $orderbox->api('GET', '/products/category-keys-mapping.json', array(), $response, 'resellerclubmdhostinghk', 'configoptions');
    
    if (in_array('multidomainhostinglinuxhk', $active_products['hosting']) || in_array('multidomainwindowshostinghk', $active_products['hosting'])) {
        $plans = $orderbox->api('GET', '/products/plan-details.json', array(), $response, 'resellerclubmdhostinghk', 'configoptions');

        if (array_key_exists('multidomainhostinglinuxhk', $plans) && in_array('multidomainhostinglinuxhk', $active_products['hosting'])) {
            $hosting_plans = $plans['multidomainhostinglinuxhk'];
            foreach ($hosting_plans as $plan_id => $plan) {
                $hosting_plan_names[] = 'Linux ' . ' - ' . $plan['plan_name'] . ' - ' . $plan_id;
            }
        }
        if (array_key_exists('multidomainwindowshostinghk', $plans) && in_array('multidomainwindowshostinghk', $active_products['hosting'])) {
            $hosting_plans = $plans['multidomainwindowshostinghk'];
            foreach ($hosting_plans as $plan_id => $plan) {
...
function resellerclubmdhostinghk_CreateAccount( $params ) {

    global $orderbox;

    try {
        $client_details = $orderbox->api( 'GET' , '/customers/details.json' , array( 'username' => $params['clientsdetails']['email'] ) , $response , 'resellerclubmdhostinghk' , 'create' );
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_order_add = '/multidomainhosting/windows/hk/add.json';
        } else {
            $api_path_order_add = '/multidomainhosting/linux/hk/add.json';
        }

        $order_api_result = $orderbox->api( 'POST' , $api_path_order_add , $order_details , $response , 'resellerclubmdhostinghk' , 'create' );
        ...
        logModuleCall('resellerclubmdhostinghk', 'create', 'unknown', 'Exception : ' . $e->getMessage() );
        return "Customer sign up error - " . $e->getMessage();
    }

}

function resellerclubmdhostinghk_SuspendAccount( $params  ) {
    ...
    try {
        $plan_pieces = _get_plan_details( $params['configoption1']);
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/hk/orderid.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/hk/orderid.json';
        }

        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response , 'resellerclubmdhostinghk' , 'suspend' );
        ...
        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            ...
            $order_suspend_result = $orderbox->api( 'POST' , '/orders/suspend.json' , array( 'order-id' => $order_id , 'reason' => $params['suspendreason'] ) , $response , 'resellerclubmdhostinghk' , 'suspend' );
        ...
        logModuleCall('resellerclubmdhostinghk', 'suspend', 'unknown', 'Exception : ' . $e->getMessage() );
        return "Order suspend error - " . $e->getMessage();
    }

}

function resellerclubmdhostinghk_UnsuspendAccount( $params  ) {
    ...
    try {
        ...
        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response , 'resellerclubmdhostinghk' , 'unsuspend' );
        ...
        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            ...
            $order_unsuspend_result = $orderbox->api( 'POST' , '/orders/unsuspend.json' , array( 'order-id' => $order_id ) , $response , 'resellerclubmdhostinghk' , 'unsuspend'  );
        ...
        logModuleCall('resellerclubmdhostinghk', 'unsuspend', 'unknown', 'Exception : ' . $e->getMessage() );
        return "Order unsuspend error - " . $e->getMessage();
    }

}

function resellerclubmdhostinghk_TerminateAccount( $params  ) {
    ...
    try {
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/hk/orderid.json';
            $api_path_order_delete = '/multidomainhosting/windows/hk/delete.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/hk/orderid.json';
            $api_path_order_delete = '/multidomainhosting/linux/hk/delete.json';
        }


        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response , 'resellerclubmdhostinghk' , 'terminate'  );

        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            return $order_id_result['message'];
        } else {
            $order_id = $order_id_result;
            $order_delete_result = $orderbox->api( 'POST' , $api_path_order_delete , array( 'order-id' => $order_id ) , $response , 'resellerclubmdhostinghk' , 'terminate'  );
...
function resellerclubmdhostinghk_Renew( $params  ) {
    ...
    try {
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/hk/orderid.json';
            $api_path_order_renew = '/multidomainhosting/windows/hk/renew.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/hk/orderid.json';
            $api_path_order_renew = '/multidomainhosting/linux/hk/renew.json';
        }

        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response  , 'resellerclubmdhostinghk' , 'renew' );
        ...
        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            ...
            $order_api_result = $orderbox->api( 'POST' , $api_path_order_renew , $order_details , $response , 'resellerclubmdhostinghk' , 'renew'  );
...
function resellerclubmdhostinghk_manualrenew( $params ) {

    //Renew the order
    $renew_result = resellerclubmdhostinghk_Renew( $params );
...
function resellerclubmdhostinghk_ChangePackage( $params ) {
    global $orderbox;
    ...
    try {
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/hk/orderid.json';
            $api_path_order_modify = '/multidomainhosting/windows/hk/modify.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/hk/orderid.json';
            $api_path_order_modify = '/multidomainhosting/linux/hk/modify.json';
        }
        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response , 'resellerclubmdhostinghk' , 'changepackage'  );
        ...
        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            ...
            $order_api_result = $orderbox->api( 'POST' , $api_path_order_modify , $order_details , $response, 'resellerclubmdhostinghk' , 'changepackage' );
...
function resellerclubmdhostinghk_ClientAreaCustomButtonArray() {
}

function resellerclubmdhostinghk_AdminCustomButtonArray() {
    return array( "Execute Manual Renew" => "manualrenew" );
}

function resellerclubmdhostinghk_ClientArea( $params ) {
    ...
    try {
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/hk/orderid.json';
            $api_path_order_details = '/multidomainhosting/windows/hk/details.json';
            $api_path_dns_details = '/multidomainhosting/windows/hk/dns-record.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/hk/orderid.json';
            $api_path_order_details = '/multidomainhosting/linux/hk/details.json';
            $api_path_dns_details = '/multidomainhosting/linux/hk/dns-record.json';
        }

        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response, 'resellerclubmdhostinghk' , 'clientarea' );

        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            $is_processing = true;
        } else {
            $order_id = $order_id_result;
            $order_details = $orderbox->api( 'GET' , $api_path_order_details , array( 'order-id' => $order_id ) , $response , 'resellerclubmdhostinghk' , 'clientarea' );
            $order_dns_details = $orderbox->api( 'GET' , $api_path_dns_details , array( 'order-id' => $order_id ) , $response , 'resellerclubmdhostinghk' , 'clientarea' );
...
function resellerclubmdhostinghk_LoginLink( $params ) {
...
function _createCustomer( $params ) {
    ...
    $create_customer_result = $orderbox->api( 'POST' , '/customers/signup.json' , $customer_details , $response_headers , 'resellerclubmdhostinghk' , 'create' );
...
if( !function_exists('_get_control_panel_link') ) {
    ...
    $client_details = $orderbox->api( 'GET' , '/customers/details.json' , array( 'username' => $params['clientsdetails']['email'] ) , $response , 'resellerclubmdhostinghk' , 'clientarea' );
    if( is_array($client_details) && strtolower( $client_details['status'] ) == 'error' ) {
        $error = "Customer ({$params['clientsdetails']['email']}) not found at Resellerclub";
    } else {
        $customer_id = $client_details['customerid'];
        $customer_temp_password = $orderbox->api( 'GET' , '/customers/temp-password.json' , array( 'customer-id' => $customer_id ) , $response , 'resellerclubmdhostinghk' , 'clientarea' );
    ...
    if( $error == '' ) {
     ...
     if( $error == '' ) {
        ...
        $reseller_details = $orderbox->api( 'GET' , '/resellers/details.json' , array() , $response , 'resellerclubmdhostinghk' , 'clientarea' );
    ...
    if( $error == '' ) {
        ...
        $authentication_token_result = $orderbox->api( 'GET' , '/customers/generate-token.json' , array( 'username' => $params['clientsdetails']['email'] , 'passwd' => $resellerclub_customer_password , 'ip' => $ip ) , $response , 'resellerclubmdhostinghk' , 'clientarea');
