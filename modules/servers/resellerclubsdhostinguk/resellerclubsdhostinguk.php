...
/**
 * WHMCS Module for Resellerclub's Single Domain Hosting - UK ( Linux / Windows )
 */
// AddOn Module Name
 ...
 */
global $orderbox;
$orderbox = new orderboxapi($resellerclub_config['resellerid'], $resellerclub_config['apikey'], $resellerclub_config['enable_log'], 'rc-whmcs-resellerclubsdhosting-uk');

/**
 * WHMCS provisional module core functions
 */
function resellerclubsdhostinguk_ConfigOptions() {

    global $orderbox;
    $hosting_plan_names = array();
    $active_products = $orderbox->api('GET', '/products/category-keys-mapping.json', array(), $response, 'resellerclubsdhostinguk', 'configoptions');

    if (in_array('singledomainhostinglinuxuk', $active_products['hosting']) || in_array('singledomainhostingwindowsuk', $active_products['hosting'])) {
        $plans = $orderbox->api('GET', '/products/plan-details.json', array(), $response, 'resellerclubsdhostinguk', 'configoptions');

        if (array_key_exists('singledomainhostinglinuxuk', $plans) && in_array('singledomainhostinglinuxuk', $active_products['hosting'])) {
            $hosting_plans = $plans['singledomainhostinglinuxuk'];
        ...
        if (array_key_exists('singledomainhostingwindowsuk', $plans) && in_array('singledomainhostingwindowsuk', $active_products['hosting'])) {
            $hosting_plans = $plans['singledomainhostingwindowsuk'];
            foreach ($hosting_plans as $plan_id => $plan) {
                if ('Active' == $plan['status']) {
...
function resellerclubsdhostinguk_CreateAccount($params) {

    global $orderbox;

    try {
        $client_details = $orderbox->api('GET', '/customers/details.json', array('username' => $params['clientsdetails']['email']), $response, 'resellerclubsdhostinguk', 'create');
...
//add new order in resellerclub
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_order_add = '/singledomainhosting/windows/uk/add.json';
        } else {
            $api_path_order_add = '/singledomainhosting/linux/uk/add.json';
        }

        $order_api_result = $orderbox->api('POST', $api_path_order_add, $order_details, $response, 'resellerclubsdhostinguk', 'create');
        ...
        logModuleCall('resellerclubsdhostinguk', 'create', 'unknown', 'Exception : ' . $e->getMessage());
        return "Customer sign up error - " . $e->getMessage();
    }
}

function resellerclubsdhostinguk_SuspendAccount($params) {
    ...
    try {
        $plan_pieces = _get_plan_details($params['configoption1']);
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/singledomainhosting/windows/uk/orderid.json';
        } else {
            $api_path_orderid_from_domain = '/singledomainhosting/linux/uk/orderid.json';
        }

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubsdhostinguk', 'suspend');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_suspend_result = $orderbox->api('POST', '/orders/suspend.json', array('order-id' => $order_id, 'reason' => $params['suspendreason']), $response, 'resellerclubsdhostinguk', 'suspend');
        ...
        logModuleCall('resellerclubsdhostinguk', 'suspend', 'unknown', 'Exception : ' . $e->getMessage());
        return "Order suspend error - " . $e->getMessage();
    }
}

function resellerclubsdhostinguk_UnsuspendAccount($params) {
    ...
    try {
        ...
        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubsdhostinguk', 'unsuspend');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_unsuspend_result = $orderbox->api('POST', '/orders/unsuspend.json', array('order-id' => $order_id), $response, 'resellerclubsdhostinguk', 'unsuspend');
        ...
        logModuleCall('resellerclubsdhostinguk', 'unsuspend', 'unknown', 'Exception : ' . $e->getMessage());
        return "Order unsuspend error - " . $e->getMessage();
    }
}

function resellerclubsdhostinguk_TerminateAccount($params) {
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/singledomainhosting/windows/uk/orderid.json';
            $api_path_order_delete = '/singledomainhosting/windows/uk/delete.json';
        } else {
            $api_path_orderid_from_domain = '/singledomainhosting/linux/uk/orderid.json';
            $api_path_order_delete = '/singledomainhosting/linux/uk/delete.json';
        }


        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubsdhostinguk', 'terminate');

        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            return $order_id_result['message'];
        } else {
            $order_id = $order_id_result;
            $order_delete_result = $orderbox->api('POST', $api_path_order_delete, array('order-id' => $order_id), $response, 'resellerclubsdhostinguk', 'terminate');
...
function resellerclubsdhostinguk_Renew($params) {
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/singledomainhosting/windows/uk/orderid.json';
            $api_path_order_renew = '/singledomainhosting/windows/uk/renew.json';
        } else {
            $api_path_orderid_from_domain = '/singledomainhosting/linux/uk/orderid.json';
            $api_path_order_renew = '/singledomainhosting/linux/uk/renew.json';
        }

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubsdhostinguk', 'renew');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_api_result = $orderbox->api('POST', $api_path_order_renew, $order_details, $response, 'resellerclubsdhostinguk', 'renew');
...
function resellerclubsdhostinguk_manualrenew($params) {

//Renew the order
    $renew_result = resellerclubsdhostinguk_Renew($params);
...
function resellerclubsdhostinguk_ChangePackage($params) {
    global $orderbox;
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/singledomainhosting/windows/uk/orderid.json';
            $api_path_order_modify = '/singledomainhosting/windows/uk/modify.json';
        } else {
            $api_path_orderid_from_domain = '/singledomainhosting/linux/uk/orderid.json';
            $api_path_order_modify = '/singledomainhosting/linux/uk/modify.json';
        }
        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubsdhostinguk', 'changepackage');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_api_result = $orderbox->api('POST', $api_path_order_modify, $order_details, $response, 'resellerclubsdhostinguk', 'changepackage');
...
function resellerclubsdhostinguk_ClientAreaCustomButtonArray() {
    
}

function resellerclubsdhostinguk_AdminCustomButtonArray() {
    return array("Execute Manual Renew" => "manualrenew");
}

function resellerclubsdhostinguk_ClientArea($params) {
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/singledomainhosting/windows/uk/orderid.json';
            $api_path_order_details = '/singledomainhosting/windows/uk/details.json';
            $api_path_dns_details = '/singledomainhosting/windows/uk/dns-record.json';
        } else {
            $api_path_orderid_from_domain = '/singledomainhosting/linux/uk/orderid.json';
            $api_path_order_details = '/singledomainhosting/linux/uk/details.json';
            $api_path_dns_details = '/singledomainhosting/linux/uk/dns-record.json';
        }

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubsdhostinguk', 'clientarea');

        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            $is_processing = true;
        } else {
            $order_id = $order_id_result;
            $order_details = $orderbox->api('GET', $api_path_order_details, array('order-id' => $order_id), $response, 'resellerclubsdhostinguk', 'clientarea');
            $order_dns_details = $orderbox->api('GET', $api_path_dns_details, array('order-id' => $order_id), $response, 'resellerclubsdhostinguk', 'clientarea');
        }
...
function resellerclubsdhostinguk_LoginLink($params) {
...
//TODO :: Set phone country code (phone-cc) appropriately
        ...
        $create_customer_result = $orderbox->api('POST', '/customers/signup.json', $customer_details, $response_headers, 'resellerclubsdhostinguk', 'create');
...
if (!function_exists('_get_control_panel_link')) {
    ...
    function _get_control_panel_link($params) {
        ...
        $client_details = $orderbox->api('GET', '/customers/details.json', array('username' => $params['clientsdetails']['email']), $response, 'resellerclubsdhostinguk', 'clientarea');
        if (is_array($client_details) && strtolower($client_details['status']) == 'error') {
            $error = "Customer ({$params['clientsdetails']['email']}) not found at Resellerclub";
        } else {
            $customer_id = $client_details['customerid'];
            $customer_temp_password = $orderbox->api('GET', '/customers/temp-password.json', array('customer-id' => $customer_id), $response, 'resellerclubsdhostinguk', 'clientarea');
...
// get reseller branded url
            $reseller_details = $orderbox->api('GET', '/resellers/details.json', array(), $response, 'resellerclubsdhostinguk', 'clientarea');
...
// generate authentication token
            ...
            $authentication_token_result = $orderbox->api('GET', '/customers/generate-token.json', array('username' => $params['clientsdetails']['email'], 'passwd' => $resellerclub_customer_password, 'ip' => $ip), $response, 'resellerclubsdhostinguk', 'clientarea');
