...
/**
 * WHMCS Module for Resellerclub's Multi Domain Hosting - India ( Linux / Windows )
 */
...
global $orderbox;
$orderbox =  new orderboxapi( $resellerclub_config['resellerid'] , $resellerclub_config['apikey'],$resellerclub_config['enable_log'], 'rc-whmcs-resellerclubmdhosting-in' );

/**
 * WHMCS provisional module core functions
 */

function resellerclubmdhostingin_ConfigOptions() {

    global $orderbox;
    $hosting_plan_names = array();
    
    $active_products = $orderbox->api('GET', '/products/category-keys-mapping.json', array(), $response, 'resellerclubmdhostingin', 'configoptions');
    
    if (in_array('multidomainhostinglinuxin', $active_products['hosting']) || in_array('multidomainwindowshostingin', $active_products['hosting'])) {
        $plans = $orderbox->api('GET', '/products/plan-details.json', array(), $response, 'resellerclubmdhostingin', 'configoptions');

        if (array_key_exists('multidomainhostinglinuxin', $plans) && in_array('multidomainhostinglinuxin', $active_products['hosting'])) {
            $hosting_plans = $plans['multidomainhostinglinuxin'];
            foreach ($hosting_plans as $plan_id => $plan) {
                $hosting_plan_names[] = 'Linux ' . ' - ' . $plan['plan_name'] . ' - ' . $plan_id;
            }
        }
        if (array_key_exists('multidomainwindowshostingin', $plans) && in_array('multidomainwindowshostingin', $active_products['hosting'])) {
            $hosting_plans = $plans['multidomainwindowshostingin'];
            foreach ($hosting_plans as $plan_id => $plan) {
...
function resellerclubmdhostingin_CreateAccount( $params ) {

    global $orderbox;

    try {
        $client_details = $orderbox->api( 'GET' , '/customers/details.json' , array( 'username' => $params['clientsdetails']['email'] ) , $response , 'resellerclubmdhostingin' , 'create' );
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_order_add = '/multidomainhosting/windows/in/add.json';
        } else {
            $api_path_order_add = '/multidomainhosting/linux/in/add.json';
        }

        $order_api_result = $orderbox->api( 'POST' , $api_path_order_add , $order_details , $response , 'resellerclubmdhostingin' , 'create' );
        ...
        logModuleCall('resellerclubmdhostingin', 'create', 'unknown', 'Exception : ' . $e->getMessage() );
        return "Customer sign up error - " . $e->getMessage();
    }

}

function resellerclubmdhostingin_SuspendAccount( $params  ) {
    ...
    try {
        $plan_pieces = _get_plan_details( $params['configoption1']);
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/in/orderid.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/in/orderid.json';
        }

        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response , 'resellerclubmdhostingin' , 'suspend' );
        ...
        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            ...
            $order_suspend_result = $orderbox->api( 'POST' , '/orders/suspend.json' , array( 'order-id' => $order_id , 'reason' => $params['suspendreason'] ) , $response , 'resellerclubmdhostingin' , 'suspend' );
        ...
        logModuleCall('resellerclubmdhostingin', 'suspend', 'unknown', 'Exception : ' . $e->getMessage() );
        return "Order suspend error - " . $e->getMessage();
    }

}

function resellerclubmdhostingin_UnsuspendAccount( $params  ) {
    ...
    try {
        ...
        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response , 'resellerclubmdhostingin' , 'unsuspend' );
        ...
        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            ...
            $order_unsuspend_result = $orderbox->api( 'POST' , '/orders/unsuspend.json' , array( 'order-id' => $order_id ) , $response , 'resellerclubmdhostingin' , 'unsuspend'  );
        ...
        logModuleCall('resellerclubmdhostingin', 'unsuspend', 'unknown', 'Exception : ' . $e->getMessage() );
        return "Order unsuspend error - " . $e->getMessage();
    }

}

function resellerclubmdhostingin_TerminateAccount( $params  ) {
    ...
    try {
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/in/orderid.json';
            $api_path_order_delete = '/multidomainhosting/windows/in/delete.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/in/orderid.json';
            $api_path_order_delete = '/multidomainhosting/linux/in/delete.json';
        }


        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response , 'resellerclubmdhostingin' , 'terminate'  );

        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            return $order_id_result['message'];
        } else {
            $order_id = $order_id_result;
            $order_delete_result = $orderbox->api( 'POST' , $api_path_order_delete , array( 'order-id' => $order_id ) , $response , 'resellerclubmdhostingin' , 'terminate'  );
...
function resellerclubmdhostingin_Renew( $params  ) {
    ...
    try {
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/in/orderid.json';
            $api_path_order_renew = '/multidomainhosting/windows/in/renew.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/in/orderid.json';
            $api_path_order_renew = '/multidomainhosting/linux/in/renew.json';
        }

        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response  , 'resellerclubmdhostingin' , 'renew' );
        ...
        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            ...
            $order_api_result = $orderbox->api( 'POST' , $api_path_order_renew , $order_details , $response , 'resellerclubmdhostingin' , 'renew'  );
...
function resellerclubmdhostingin_manualrenew( $params ) {

    //Renew the order
    $renew_result = resellerclubmdhostingin_Renew( $params );
...
function resellerclubmdhostingin_ChangePackage( $params ) {
    global $orderbox;
    ...
    try {
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/in/orderid.json';
            $api_path_order_modify = '/multidomainhosting/windows/in/modify.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/in/orderid.json';
            $api_path_order_modify = '/multidomainhosting/linux/in/modify.json';
        }
        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response , 'resellerclubmdhostingin' , 'changepackage'  );
        ...
        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            ...
            $order_api_result = $orderbox->api( 'POST' , $api_path_order_modify , $order_details , $response, 'resellerclubmdhostingin' , 'changepackage' );
...
function resellerclubmdhostingin_ClientAreaCustomButtonArray() {
}

function resellerclubmdhostingin_AdminCustomButtonArray() {
    return array( "Execute Manual Renew" => "manualrenew" );
}

function resellerclubmdhostingin_ClientArea( $params ) {
    ...
    try {
        ...
        if( 'windows' == $plan_pieces['type'] ) {
            $api_path_orderid_from_domain = '/multidomainhosting/windows/in/orderid.json';
            $api_path_order_details = '/multidomainhosting/windows/in/details.json';
            $api_path_dns_details = '/multidomainhosting/windows/in/dns-record.json';
        } else {
            $api_path_orderid_from_domain = '/multidomainhosting/linux/in/orderid.json';
            $api_path_order_details = '/multidomainhosting/linux/in/details.json';
            $api_path_dns_details = '/multidomainhosting/linux/in/dns-record.json';
        }

        $order_id_result = $orderbox->api( 'GET' , $api_path_orderid_from_domain , array( 'domain-name' => $params['domain'] ) , $response, 'resellerclubmdhostingin' , 'clientarea' );

        if( is_array( $order_id_result ) && array_key_exists( 'status', $order_id_result ) && strtolower( $order_id_result['status'] ) == 'error'  ) {
            $is_processing = true;
        } else {
            $order_id = $order_id_result;
            $order_details = $orderbox->api( 'GET' , $api_path_order_details , array( 'order-id' => $order_id ) , $response , 'resellerclubmdhostingin' , 'clientarea' );
            $order_dns_details = $orderbox->api( 'GET' , $api_path_dns_details , array( 'order-id' => $order_id ) , $response , 'resellerclubmdhostingin' , 'clientarea' );
...
function resellerclubmdhostingin_LoginLink( $params ) {
...
function _createCustomer( $params ) {
    ...
    $create_customer_result = $orderbox->api( 'POST' , '/customers/signup.json' , $customer_details , $response_headers , 'resellerclubmdhostingin' , 'create' );
...
if( !function_exists('_get_control_panel_link') ) {
    ...
    $client_details = $orderbox->api( 'GET' , '/customers/details.json' , array( 'username' => $params['clientsdetails']['email'] ) , $response , 'resellerclubmdhostingin' , 'clientarea' );
    if( is_array($client_details) && strtolower( $client_details['status'] ) == 'error' ) {
        $error = "Customer ({$params['clientsdetails']['email']}) not found at Resellerclub";
    } else {
        $customer_id = $client_details['customerid'];
        $customer_temp_password = $orderbox->api( 'GET' , '/customers/temp-password.json' , array( 'customer-id' => $customer_id ) , $response , 'resellerclubmdhostingin' , 'clientarea' );
    ...
    if( $error == '' ) {
     ...
     if( $error == '' ) {
        ...
        $reseller_details = $orderbox->api( 'GET' , '/resellers/details.json' , array() , $response , 'resellerclubmdhostingin' , 'clientarea' );
    ...
    if( $error == '' ) {
        ...
        $authentication_token_result = $orderbox->api( 'GET' , '/customers/generate-token.json' , array( 'username' => $params['clientsdetails']['email'] , 'passwd' => $resellerclub_customer_password , 'ip' => $ip ) , $response , 'resellerclubmdhostingin' , 'clientarea');
