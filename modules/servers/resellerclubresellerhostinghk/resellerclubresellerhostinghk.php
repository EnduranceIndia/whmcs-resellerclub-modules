...
/**
 * WHMCS Module for Resellerclub's Reseller Hosting - Hong Kong ( Linux / Windows )
 */
// AddOn Module Name
 ...
 */
global $orderbox;
$orderbox = new orderboxapi($resellerclub_config['resellerid'], $resellerclub_config['apikey'], $resellerclub_config['enable_log'], 'rc-whmcs-resellerclubresellerhosting-hk');

/**
 * WHMCS provisional module core functions
 */
function resellerclubresellerhostinghk_ConfigOptions() {

    global $orderbox;
    $hosting_plan_names = array();
    $active_products = $orderbox->api('GET', '/products/category-keys-mapping.json', array(), $response, 'resellerclubresellerhostinghk', 'configoptions');

    if (in_array('resellerhostinglinuxhk', $active_products['hosting']) || in_array('resellerwindowshostinghk', $active_products['hosting'])) {
        $plans = $orderbox->api('GET', '/products/plan-details.json', array(), $response, 'resellerclubresellerhostinghk', 'configoptions');

        if (array_key_exists('resellerhostinglinuxhk', $plans) && in_array('resellerhostinglinuxhk', $active_products['hosting'])) {
            $hosting_plans = $plans['resellerhostinglinuxhk'];
            foreach ($hosting_plans as $plan_id => $plan) {
                $hosting_plan_names[] = 'Linux ' . ' - ' . $plan['plan_name'] . ' - ' . $plan_id;
            }
        }
        if (array_key_exists('resellerwindowshostinghk', $plans) && in_array('resellerwindowshostinghk', $active_products['hosting'])) {
            $hosting_plans = $plans['resellerwindowshostinghk'];
            foreach ($hosting_plans as $plan_id => $plan) {
...
function resellerclubresellerhostinghk_CreateAccount($params) {

    global $orderbox;

    try {
        $client_details = $orderbox->api('GET', '/customers/details.json', array('username' => $params['clientsdetails']['email']), $response, 'resellerclubresellerhostinghk', 'create');
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_order_add = '/resellerhosting/windows/hk/add.json';
        } else {
            $api_path_order_add = '/resellerhosting/linux/hk/add.json';
        }

        $order_api_result = $orderbox->api('POST', $api_path_order_add, $order_details, $response, 'resellerclubresellerhostinghk', 'create');
        ...
        logModuleCall('resellerclubresellerhostinghk', 'create', 'unknown', 'Exception : ' . $e->getMessage());
        return "Customer sign up error - " . $e->getMessage();
    }
}

function resellerclubresellerhostinghk_SuspendAccount($params) {
    ...
    try {
        $plan_pieces = _get_plan_details($params['configoption1']);
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/hk/orderid.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/hk/orderid.json';
        }

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostinghk', 'suspend');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_suspend_result = $orderbox->api('POST', '/orders/suspend.json', array('order-id' => $order_id, 'reason' => $params['suspendreason']), $response, 'resellerclubresellerhostinghk', 'suspend');
        ...
        logModuleCall('resellerclubresellerhostinghk', 'suspend', 'unknown', 'Exception : ' . $e->getMessage());
        return "Order suspend error - " . $e->getMessage();
    }
}

function resellerclubresellerhostinghk_UnsuspendAccount($params) {
    ...
    try {
        ...
        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostinghk', 'unsuspend');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_unsuspend_result = $orderbox->api('POST', '/orders/unsuspend.json', array('order-id' => $order_id), $response, 'resellerclubresellerhostinghk', 'unsuspend');
        ...
        logModuleCall('resellerclubresellerhostinghk', 'unsuspend', 'unknown', 'Exception : ' . $e->getMessage());
        return "Order unsuspend error - " . $e->getMessage();
    }
}

function resellerclubresellerhostinghk_TerminateAccount($params) {
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/hk/orderid.json';
            $api_path_order_delete = '/resellerhosting/windows/hk/delete.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/hk/orderid.json';
            $api_path_order_delete = '/resellerhosting/linux/hk/delete.json';
        }


        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostinghk', 'terminate');

        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            return $order_id_result['message'];
        } else {
            $order_id = $order_id_result;
            $order_delete_result = $orderbox->api('POST', $api_path_order_delete, array('order-id' => $order_id), $response, 'resellerclubresellerhostinghk', 'terminate');
...
function resellerclubresellerhostinghk_Renew($params) {
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/hk/orderid.json';
            $api_path_order_renew = '/resellerhosting/windows/hk/renew.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/hk/orderid.json';
            $api_path_order_renew = '/resellerhosting/linux/hk/renew.json';
        }

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostinghk', 'renew');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_api_result = $orderbox->api('POST', $api_path_order_renew, $order_details, $response, 'resellerclubresellerhostinghk', 'renew');
...
function resellerclubresellerhostinghk_manualrenew($params) {

    //Renew the order
    $renew_result = resellerclubresellerhostinghk_Renew($params);
...
function resellerclubresellerhostinghk_ChangePackage($params) {
    global $orderbox;
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/hk/orderid.json';
            $api_path_order_modify = '/resellerhosting/windows/hk/modify.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/hk/orderid.json';
            $api_path_order_modify = '/resellerhosting/linux/hk/modify.json';
        }
        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostinghk', 'changepackage');
        ...
        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            ...
            $order_api_result = $orderbox->api('POST', $api_path_order_modify, $order_details, $response, 'resellerclubresellerhostinghk', 'changepackage');
...
function resellerclubresellerhostinghk_ClientAreaCustomButtonArray() {
    
}

function resellerclubresellerhostinghk_AdminCustomButtonArray() {
    return array("Execute Manual Renew" => "manualrenew");
}

function resellerclubresellerhostinghk_ClientArea($params) {
    ...
    if (isset($_POST['cplogin']) && strlen(trim($_POST['cplogin'])) > 0) {
        ...
        switch ($cplogin_action) {
            ...
            case 'whm': _redirect_to_whm_control_panel($params);
                break;
            case 'whmcs_license': resellerclubresellerhostinghk_GenerateLicense($params);
                break;
        }
    }
    ...
    try {
        ...
        if ('windows' == $plan_pieces['type']) {
            $api_path_orderid_from_domain = '/resellerhosting/windows/hk/orderid.json';
            $api_path_order_details = '/resellerhosting/windows/hk/details.json';
            $api_path_dns_details = '/resellerhosting/windows/hk/dns-record.json';
        } else {
            $api_path_orderid_from_domain = '/resellerhosting/linux/hk/orderid.json';
            $api_path_order_details = '/resellerhosting/linux/hk/details.json';
            $api_path_dns_details = '/resellerhosting/linux/hk/dns-record.json';
        }

        $order_id_result = $orderbox->api('GET', $api_path_orderid_from_domain, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostinghk', 'clientarea');

        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            $is_processing = true;
        } else {
            $order_id = $order_id_result;
            $order_details = $orderbox->api('GET', $api_path_order_details, array('order-id' => $order_id), $response, 'resellerclubresellerhostinghk', 'clientarea');
            $order_dns_details = $orderbox->api('GET', $api_path_dns_details, array('order-id' => $order_id), $response, 'resellerclubresellerhostinghk', 'clientarea');
        }
        ...
        if ($is_processing) {
            ...
            if ('windows' == $plan_pieces['type']) {
                ...
                $plans = $orderbox->api('GET', '/products/plan-details.json', array(), $response, 'resellerclubresellerhostinghk', 'clientarea');
            }
            ...
            if ($plans['resellerhostinglinuxhk'][$plan_pieces['id']]['includes_whmcs'] == true) {
...
function resellerclubresellerhostinghk_LoginLink($params) {
    echo "<strong>Do Not Modify</strong>" . _display_control_panel_link($params);
}

function resellerclubresellerhostinghk_GenerateLicense($params) {
    ...
    try {
        $plan_pieces = _get_plan_details($params['configoption1']);
        if ('windows' == $plan_pieces['type']) {
            $api_path = '/resellerhosting/windows/hk/orderid.json';
            $api_path_generate_license_key = '/resellerhosting/windows/hk/generate-license-key.json';
        } else {
            $api_path = '/resellerhosting/linux/hk/orderid.json';
            $api_path_generate_license_key = '/resellerhosting/linux/hk/generate-license-key.json';
        }
        $order_id_result = $orderbox->api('GET', $api_path, array('domain-name' => $params['domain']), $response, 'resellerclubresellerhostinghk', 'generateLicense');

        if (is_array($order_id_result) && array_key_exists('status', $order_id_result) && strtolower($order_id_result['status']) == 'error') {
            return $order_id_result['message'];
        } else {
            $order_id = $order_id_result;
            $license_details = $orderbox->api('POST', $api_path_generate_license_key, array('order-id' => $order_id), $response, 'resellerclubresellerhostinghk', 'generateLicense');
...
if (!function_exists('_createCustomer')) {
    ...
    function _createCustomer($params) {
        ...
        $create_customer_result = $orderbox->api('POST', '/customers/signup.json', $customer_details, $response_headers, 'resellerclubresellerhostinghk', 'create');
...
if (!function_exists('_get_control_panel_link')) {
    ...
    function _get_control_panel_link($params) {
        ...
        $client_details = $orderbox->api('GET', '/customers/details.json', array('username' => $params['clientsdetails']['email']), $response, 'resellerclubresellerhostinghk', 'clientarea');
        if (is_array($client_details) && strtolower($client_details['status']) == 'error') {
            $error = "Customer ({$params['clientsdetails']['email']}) not found at Resellerclub";
        } else {
            $customer_id = $client_details['customerid'];
            $customer_temp_password = $orderbox->api('GET', '/customers/temp-password.json', array('customer-id' => $customer_id), $response, 'resellerclubresellerhostinghk', 'clientarea');
        ...
        if ($error == '') {
            ...
            $reseller_details = $orderbox->api('GET', '/resellers/details.json', array(), $response, 'resellerclubresellerhostinghk', 'clientarea');
        ...
        if ($error == '') {
            ...
            $authentication_token_result = $orderbox->api('GET', '/customers/generate-token.json', array('username' => $params['clientsdetails']['email'], 'passwd' => $resellerclub_customer_password, 'ip' => $ip), $response, 'resellerclubresellerhostinghk', 'clientarea');
